...
async function scrapeListingPage(page: Page, url: string): Promise<ScrapedListing | null> {
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      const getAll = (selector: string) => Array.from(document.querySelectorAll(selector)).map(el => el.textContent?.trim() || '');
    ...
    if (addressLower.includes('brooklyn')) borough = 'Brooklyn';
    else if (addressLower.includes('queens')) borough = 'Queens';
    else if (addressLower.includes('bronx')) borough = 'Bronx';
    else if (addressLower.includes('staten island')) borough = 'Staten Island';
